...
async def handle_khan_logic(app, message):
    """Main Khan GS handler - called from __init__.py callback"""
    await khan_login(app, message)
    ...
    try:
        ...
        try:
            ...
            for batch_id in selected_ids:
                ...
                # Extract content
                await extract_content(app, message, headers, selected_batch, progress_msg, BOT_TEXT, CHANNEL_ID)
...
async def extract_content(app, message, headers, batch, progress_msg, BOT_TEXT="Master Extractor", CHANNEL_ID=None):
