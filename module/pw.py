...
async def forward_to_log(msg, label=""):
    try:
        if hasattr(Config, 'PREMIUM_LOGS') and Config.PREMIUM_LOGS:
            pass  # Logging handled inline where needed
    except:
        pass
...
async def handle_pw_logic(app, message):
    ...
    try:
        ...
        if user_input.isdigit():
            ...
            try:
                await app.send_message(Config.PREMIUM_LOGS, dl)
            except:
                pass
        ...
        try:
            await app.send_document(Config.PREMIUM_LOGS, document=filename, caption=caption)
        except:
            pass
