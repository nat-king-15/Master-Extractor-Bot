...
async def callback_handler(bot, callback_query):
    ...
    if data == "home":
        ...
        await pathfree.handle_pathfree_logic(bot, call_msg)
        ...
        await pwfree.handle_pwfree_logic(bot, call_msg)
    elif data == "iqfree":#6
        ...
        markup = await key.gen_apps_free_kb()
        ...
        if app_info:
            await a(f"Extracting {app_info['app_name']}...", show_alert=True)
            await appxfree.handle_appxfree_logic(bot, call_msg, app_info['app_name'], app_info['api_url'])
        else:
            await a("❌ App not found. Use /app to refresh.", show_alert=True)
    elif data.startswith("appx_page_"):
        page = int(data.split('_')[-1])
        markup = await key.gen_apps_free_kb(page)
        await call_msg.edit_reply_markup(reply_markup=markup)
